...
In this lab, we’ll change our shopping cart and order implementation and add a payment service as an Event-Driven/Reactive application in our cloud-native appliation architecture. These cloud-native applications will use AMQ Streams (based on Apache Kafka) as a messaging/streaming backbone. AMQ Streams makes it easy to run Apache Kafka on OpenShift with key features:
...
Open the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-cloudnativeapps[Topology View for the {{ USER_ID }}-cloudnativeapps project^].

Next click *+Add* on the left, on the _From Catalog_ box on the project overview:
...
Click on *Create* and you will enter YAML editor that defines a *Kafka* Cluster. Keep the all values as-is then click on the *Create* button (on the bottom):
...
Click on *Create* and you will enter YAML editor that defines a *KafkaTopic* object. Change the name to `orders` as shown then click on the *Create* button (on the bottom):
...
Change the name to `payments` then click on the *Create* button (on the bottom).
...
Now we're ready to build and deploy our payment-service.  The OpenShift extension will take care of the build of the container and deployment of that build _for our current project_.  So first we need to make sure we're targeting the project to which we want to deploy this service:
...
[NOTE]
====
Once we changed to the `{{ USER_ID }}-cloudnativeapps` project with the `oc project` command we will remain on that project until we issue another `oc project` command or override this by specifying a namespace (e.g. with a `-n` parameter) on participating commands.  

For the rest of the builds in this chapter, it assumes that `{{ USER_ID }}-cloudnativeapps` is the targeted project.
====

With the proper project targeted, now we can build and deploy the project using using the OpenShift extension, which will use the maven plugin to deploy via CodeReady Workspaces Terminal.  
...
This shows that the `payment-service` can successfully process orders in the backend and send them to a (in this case) notional external payment processor and post the results to the `payments` topic
...
[arabic]
. Add a _Quarkus T-shirt_ to your cart by click on *Add to Cart*. You will see the `Success! Added!` message under the top menu.
